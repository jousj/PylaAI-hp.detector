...
# ── Глобальный перехватчик крашей ──────────────────────────────────────────

# 1. Падения в фоновых потоках
def _thread_excepthook(args):
    print("\n" + "="*60)
    print(f"[!!! CRASH В ПОТОКЕ !!!] поток: {args.thread.name}")
    print(f"Тип: {args.exc_type.__name__}: {args.exc_value}")
    print("="*60)
    traceback.print_exception(args.exc_type, args.exc_value, args.exc_tb)
    print("="*60 + "\n")
    # Убиваем весь процесс чтобы краш был виден
    os._exit(1)

threading.excepthook = _thread_excepthook

# 2. Необработанные исключения в главном потоке
...
# 3. faulthandler — ловит segfault / stack overflow / SIGABRT из C-расширений
_crash_log = open("crash_log.txt", "w")
...
def pyla_main(data):
    class Main:
        ...
        def restart_brawl_stars(self):
            print("[RESTART] restart_brawl_stars() вызван")
            ...
            if self.window_controller.device.app_current().package != window_controller.BRAWL_STARS_PACKAGE:
                ...
                try:
                    ...
                    loop.close()
                print("[RESTART] Bot got stuck. Shutting down via sys.exit(1)")
                self.window_controller.keys_up(list("wasd"))
                self.window_controller.close()
                sys.exit(1)
            print("[RESTART] завершён успешно")
            ...
            while True:
                try:
                    ...
                    if last_ft > 0 and (time.time() - last_ft) > self.window_controller.FRAME_STALE_TIMEOUT:
                        ...
                        print("[STALE] Stale frame detected -- restarting the game.")
                        self.window_controller.restart_brawl_stars()
                    ...
                    print("\n" + "="*60)
                    print(f"[!!! SystemExit !!!] код выхода: {e.code}")
                    traceback.print_exc()
                    print("="*60 + "\n")
                    raise
